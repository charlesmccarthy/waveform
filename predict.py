...
import gc  # Forcing garbage collection
...
# --- NEW HELPER FUNCTION TO MANAGE MEMORY SCOPE ---
def _run_prediction(
    audio_file: Path,
    image_file: Path,
    dot_size: int,
    ...
    pulse_smoothing: float,
    zoom_start: float,
    zoom_end: float
) -> Path:
    """
    This function contains all the logic. All large variables (y, img, frames)
    are local to this function and will be destroyed when it returns,
    ...
    # We wrap the moviepy logic in its own try/finally
    try:
        print("Loading audio file now...")
        ...
        print("Finished loading background image.")
        ...
        print(f"Total frames to generate: {n_frames}")
        ...
        print("Finished calculating amplitudes.")

        frame = np.zeros((height, width, 3), dtype=np.uint8)
        overlay = np.zeros_like(frame)
        frames = [] # This list will be huge, but it's local
        ...
        for frame_idx in range(n_frames):
            ...
            if image_file and img is not None:
                ...
                M = np.float32([[final_scale, 0, offset_x], [0, final_scale, offset_y]])
                cv2.warpAffine(img, M, (width, height), dst=frame, borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0))
            else:
                frame.fill(0)
        ...
        # This cleanup block is for the *inner* function
        print("Cleaning up moviepy video and audio clips...")
        if audio_clip:
            audio_clip.close()
        if video_clip:
            video_clip.close()
        # All other large variables (y, frames, img)
        # will now go out of scope and be destroyed.


# --- Predictor Class (now a simple wrapper) ---
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        This function now wraps the main logic and handles global cleanup.
        """
        
        print("--- Predictor.predict() method entered. ---")
        
        try:
            # Call the helper function to do all the work
            result_path = _run_prediction(
                audio_file, image_file, dot_size, dot_spacing, height, width,
                max_height, dot_color, fps, image_effect, pulse_intensity,
                pulse_smoothing, zoom_start, zoom_end
            )
            
            print("--- Predictor.predict() method finished. ---")
            return result_path
            ...
            traceback.print_exc()
            raise e # Re-raise the exception so Replicate sees it
            
        finally:
            # This is the *outer* cleanup, which runs AFTER
            # _run_prediction() has returned and all its local
            # variables (y, frames, img) are *gone*.
            print("--- Starting global cleanup ---")
            
            print("Clearing librosa cache...")
            librosa.cache.clear()
